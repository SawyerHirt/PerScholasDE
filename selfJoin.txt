...
FROM route a JOIN route b ON
  (a.company=b.company AND a.num=b.num)
WHERE a.stop LIKE (SELECT id FROM stops 
                   WHERE name LIKE 'Craiglockhart')
AND b.stop LIKE (SELECT id FROM stops 
                 WHERE name LIKE 'Tollcross')
...
SELECT DISTINCT d.name, a.company, a.num FROM
...
SELECT DISTINCT ab1.num, ab1.company, stops.name, ab2.num, ab2.company
FROM
...
ON (ab1.stop = ab2.stop)
JOIN stops ON(stops.id = ab1.stop)
